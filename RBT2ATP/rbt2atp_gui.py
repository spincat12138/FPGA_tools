...
class RBT2ATP(QtWidgets.QMainWindow):
    ...
    def _setup_table_view(self):
        ...
        for row_index in range(self.tableWidget.rowCount()):
            header_item = self.tableWidget.verticalHeaderItem(row_index)
            if header_item is not None:
                header_item.setTextAlignment(QtCore.Qt.AlignCenter)
            for column_index in range(self.tableWidget.columnCount()):
                self._align_table_item(self.tableWidget.item(row_index, column_index))

    def _align_table_item(self, item):
        if item is not None:
