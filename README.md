...
| RBT 文件整理 | `RBT_Organization/` | 扫描目录中的 `.rbt` 文件并整理复制到 `rbt/` 子目录，可选同步整理 `.bit` 文件到 `bit/` 子目录 |
...
```powershell
python .\RBT_Organization\rbt_file_organization.py <待整理目录>
```
...
- `.rbt` 扩展名匹配不区分大小写。
- 扫描时跳过输出目录，避免重复整理。
- 多个源文件会生成同一个目标文件名时，会按源文件相对路径继续展开子目录来消除重名，并在输出目录中保留对应目录结构。
- 目标文件已存在时允许覆盖，并在结果中标记。
