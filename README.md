...
`FPGA-tools` 是一个个人 FPGA 辅助工具箱。项目用统一的 PyQt5 主界面管理多个子工具，每个工具以独立标签页运行，也保留部分独立命令行或 GUI 入口，适合在 FPGA 日常开发、码流处理和配置板调试流程中复用。
...
主界面会按 `tools_registry.py` 中的注册顺序加载所有启用工具。单个子工具加载失败时，主界面仍会打开，并在对应标签页展示错误信息。
...
- 主界面只负责应用壳、标签页加载、公共服务和错误展示，不实现具体 FPGA 文件格式逻辑。
